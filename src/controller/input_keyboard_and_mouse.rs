...
use crate::{config::UserAction, player::OwningPlayer, GGRSConfig};
...
pub fn map_player_input_to_controller_input(
    ...
    for (OwningPlayer(player_handle), mut controller_input) in query.iter_mut() {
        ...
        if let ggrs::InputStatus::Disconnected = status {
            log::warn!(
                "Player {player_handle} has disconnected but the controller isn't despawned!"
            );
            continue;
        }
        ...
        let x = match (
            player_input.buttons.get(UserAction::MoveRight),
            player_input.buttons.get(UserAction::MoveLeft),
        ) {
            (true, false) => 1.0,
            (false, true) => -1.0,
            _ => 0.0,
        ...
        let z = match (
            player_input.buttons.get(UserAction::MoveForward),
            player_input.buttons.get(UserAction::MoveBackward),
        ) {
            (true, false) => 1.0,
            (false, true) => -1.0,
            _ => 0.0,
        ...
        controller_input.movement = Vec3::new(x, 0.0, z);
